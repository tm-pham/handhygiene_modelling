...
# ==================================================================== #
# Documentation is similar to Roxygen but package is not used here 
...
# ========================================================= #
# Generate times according to distribution
# ========================================================= #
...
# Log-normal distributed time-to-event until time t_end
# @param t_end: time period
...
# ========================================================= #
household.fun <- function(t_end=24, 
                          CT = 2, 
                          c_mean=1, 
                          c_sd=1, 
                          halflife=1, 
  ...
  if(CT==1){ # Fixed times (constant)
    ...
    while(length(t_c)==0){ # workaround if, due to chance, t_c is empty
      seed <- seed+10
      t_c <- exp.times(t_end,1/c_mean,seed)$t
    }
  ...
  for(d in 1:length(d_rate)){
    ...
    for(i in 1:length(t_f)){
      # Which hand contamination events before the current face-touch?
      ind <- which(t_c<=t_f[i])       
      if(length(ind)>0){
        # Last contamination event/time before current face-touch
        last <- t_c[ind[length(ind)]] 
        if(HW==0){# No handwashing events
          v[d,i] <- exp(-d_rate[d]*(t_f[i]-last)) # Prob. of persistence
          temp_exp <- c(temp_exp, t_f[i]-last) # Time of exposure
          temp_last_tc <- c(temp_last_tc, last) # Contamination time before face-touch
        }else{
          # Was there a hand washing in between contamination and face-touching?
          ind_HW <- intersect(which(t_HW>=last), which(t_HW<t_f[i]))
          if(length(ind_HW)>0){# yes: prob. of persistence is reduced
          ...
          }else{ # no hand wash before last contamination event
...
compute.eps <- function(t_end=24,c_mean=1,f_rate=10,halflife=1/60,SAR=0.5,seed=12345){
  ...
  t_f <- exp.times(t_end, f_rate, seed+2)$t
  # v = prob. of persistence at face touching events
  v <- matrix(rep(0, length(t_f)*length(d_rate)),nrow=length(d_rate)) 
  for(d in 1:length(d_rate)){
    for(i in 1:length(t_f)){
      ind <- which(t_c<=t_f[i])
      if(length(ind)>0){
...
sar.hw <- function(dc=seq(1,60)/60, 
  ...
  for(h in 1:length(hw)){
    ...
    for(s in seed:(seed+it-1)){
      eps <- compute.eps(t_end=infPeriod,c_mean=c_mean,f_rate=f_rate,halflife=dc,SAR=SAR.nohw, seed=s)
      if(HW.opt%in%c(1,2)){
        sim <- household.fun(t_end=infPeriod,CT=CT,c_mean=c_mean,HW=HW.opt,hw_mean=hw[h],f_rate=f_rate,eps=eps,halflife=dc,seed=s)
        pinf[i,] <- sim$p_inf
        numHW[i] <- length(sim$t_HW)
      }else{
        sim <- household.fun(t_end=infPeriod,CT=CT,c_mean=c_mean,HW=HW.opt,t_delay=hw[h],f_rate=f_rate,eps=eps,halflife=dc,seed=s)
        pinf[i,] <- sim$p_inf
        numHW[i] <- length(sim$t_HW)
      }
      i <- i + 1
  ...
  return(list(dataInf=dataInf, num_hw=num_hw))
}
