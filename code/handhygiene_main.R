...
# =============================================================================== #
# How transmission prob changes with hand hygiene frequency as a function of 
# duration of contamination (but where parameters are constrained to give the 
...
(num_hw_reg <- dataInf$num_hw)/(24*infPeriod) # Number of hand washes
...
(num_hw_delay <- dataInfDel$num_hw)/(24*infPeriod) # Number of hand washes
...
p2 <- ggplot(data=numDel, aes(x=reorder(freq,-as.numeric(count)),y=as.numeric(count))) +
      geom_bar(stat="identity") + 
